        ...
        <div class="dropdown ml-auto sm:ml-0">
          ...
          <div class="dropdown-box w-40">
            <div class="dropdown-box__content box dark:bg-dark-1 p-2">
              ...
              >
                <i data-feather="circle" class="w-4 h-4 mr-2" /> New Limit Plan
              </a>
              <a
                class="flex items-center block p-2 transition duration-300 ease-in-out bg-white dark:bg-dark-1 hover:bg-gray-200 dark:hover:bg-dark-2 rounded-md"
              >
                <i data-feather="box" class="w-4 h-4 mr-2" /> New Limit
              </a>
            </div>
          </div>
        </div>
...
export default {
  ...
  computed: {
    ...mapState({
      limitplans: (state) => state.limitplan.limitplans,
    }),
    ...mapGetters(["getLimitPlans"]),
  },
  mounted() {
   
    this.fetchAllLimitPlans().then(() => {
      let tableData = this.limitplans;
  ...
  methods: {
    ...mapActions(["fetchAllLimitPlans", "addBranch", "deleteBank", "updateBranch"]),
    save() {
      if (this.edit == false) {
