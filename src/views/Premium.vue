        ...
        <div class="dropdown ml-auto sm:ml-0">
          ...
          <div class="dropdown-box w-40">
            <div class="dropdown-box__content box dark:bg-dark-1 p-2">
              ...
              >
                <i data-feather="disc" class="w-4 h-4 mr-2" /> New Plan
              </a>
              <a
                class="flex items-center block p-2 transition duration-300 ease-in-out bg-white dark:bg-dark-1 hover:bg-gray-200 dark:hover:bg-dark-2 rounded-md"
              >
                <i data-feather="framer" class="w-4 h-4 mr-2" /> New Premium
              </a>
            </div>
          </div>
        </div>
...
export default {
  ...
  computed: {
    ...mapState({
      premiums: (state) => state.premium.premiums,
    }),
    ...mapGetters(["getPremiums"]),
  },
  mounted() {
   
    this.fetchAllPremiums().then(() => {
      let tableData = this.premiums;
      this.table = new Tabulator(this.$refs.table, { 
        ...
        columns: [
          ...
          {
            title: "GROUP DEPENDENTS PREMIUM",
            field: "group_dependent",
            hozAlign: "center",
            vertAlign: "middle",
            print: false,
            download: false,
          ...
          // For print format
 {
  ...
  methods: {
    ...mapActions(["fetchAllPremiums", "addBranch", "deleteBank", "updateBranch"]),
    save() {
      if (this.edit == false) {
